...
p1 <- p1 + scale_y_continuous (breaks=seq(0, 25, 2))
...
p3 <- p3 + coord_cartesian(ylim=c(3435,3535)) + scale_y_continuous(breaks = seq(3435,3535, by=10))
...
p2 <- p2 + scale_y_continuous (breaks=seq(0, 20, 2))
...
p4 <- p4 + coord_cartesian(ylim=c(3265,3335)) + scale_y_continuous(breaks = seq(3265,3335, by=5))
...
annotate_figure(plot, top = text_grob("Cell-Specific PRS association with CSF pTau217: BioFINDER-1", color = "black", face = "bold", size = 16, family = "serif"))
