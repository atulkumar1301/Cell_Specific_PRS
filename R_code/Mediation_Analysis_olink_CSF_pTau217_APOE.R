...
for (j in cells){
  ...
  sink(paste0("/Volumes/ATUL_6TB/Work/Projects/Cell_Specific_PRSs/Full_Data/Olink_Mediation/APOE/", j, "/", args[2], "/Total_Effect.txt"))
  print(summary(Total_Effect))
  sink()
  
  Mediator_Effect <-lm(Total_Original ~ NORMSCORE + Age + Gender + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data=data2)
  
  sink(paste0("/Volumes/ATUL_6TB/Work/Projects/Cell_Specific_PRSs/Full_Data/Olink_Mediation/APOE/", j, "/", args[2], "/Mediator_Effect.txt"))
  print(summary(Mediator_Effect))
  sink()
  
  Main_Effect <-lm(PHENO ~ NORMSCORE + Total_Original + Age + Gender + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, data=data2)
  sink(paste0("/Volumes/ATUL_6TB/Work/Projects/Cell_Specific_PRSs/Full_Data/Olink_Mediation/APOE/", j, "/", args[2], "/Main_Effect.txt"))
  print(summary(Main_Effect))
  sink()
  
  Mediation_Result <- mediate(Mediator_Effect, Main_Effect, treat='NORMSCORE', mediator='Total_Original', boot=T)
  sink(paste0("/Volumes/ATUL_6TB/Work/Projects/Cell_Specific_PRSs/Full_Data/Olink_Mediation/APOE/", j, "/", args[2], "/Mediation_Result.txt"))
