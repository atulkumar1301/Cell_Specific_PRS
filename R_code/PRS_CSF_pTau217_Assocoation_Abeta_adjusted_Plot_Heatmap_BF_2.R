...
df <- fread ("/Volumes/ATUL_6TB/Work/Projects/Cell_Specific_PRSs/Full_Data/Manuscript/PRS_Plots/BF_2/PRS_Plot_APOE_CSF_pTau217_Abeta_Adjusted_BF_2.txt")
...
#### Non-APOE region
df_1 <- fread ("/Volumes/ATUL_6TB/Work/Projects/Cell_Specific_PRSs/Full_Data/Manuscript/PRS_Plots/BF_2/PRS_Plot_Non_APOE_CSF_pTau217_Abeta_Adjusted.txt")
...
df <- fread ("/Volumes/ATUL_6TB/Work/Projects/Cell_Specific_PRSs/Full_Data/Cell_Type_Heat_Map/pTau_217_Abeta_Adjusted_APOE_BF_2.txt")
melted_df <- melt (df)
ggheatmap <- ggplot (data = melted_df, aes (x = Cell_Types, y = variable, fill = value)) + 
  geom_tile (color = "white") +
  scale_fill_gradient2(low = "#E69F00", high = "#009E73", midpoint = 0.7, limit = c(0.5,0.9), space = "Lab", name="Mediation Percentage")+
  xlab("Cell Types") +
  ylab("PRS Models") +
...
df_1 <- fread ("/Volumes/ATUL_6TB/Work/Projects/Cell_Specific_PRSs/Full_Data/Cell_Type_Heat_Map/pTau_217_Abeta_Adjusted_Non-APOE_BF_2.txt")
melted_df_1 <- melt (df_1)
ggheatmap1 <- ggplot (data = melted_df_1, aes (x = Cell_Types, y = variable, fill = value)) + 
  geom_tile (color = "white") +
  scale_fill_gradient2(low = "#E69F00", high = "#009E73", midpoint = 0.6, limit = c(0.4,1), space = "Lab", name="Mediation Percentage")+
  xlab("Cell Types") +
  ylab("PRS Models") +
...
heat <- ggarrange (ggheatmap, ggheatmap1, ncol = 2, nrow = 1, common.legend = FALSE, legend = "bottom")

plot <- ggarrange (p, heat, ncol = 1, nrow = 2)

annotate_figure(plot, top = text_grob("Cell-Specific PRS association with CSF pTau217 adjusted/mediated for A", color = "black", face = "bold", size = 16, family = "serif"))
