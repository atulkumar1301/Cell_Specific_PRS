...
df <- fread ("/Volumes/ATUL_6TB/Work/Projects/Cell_Specific_PRSs/Full_Data/Manuscript/PRS_Plots/BF_2/PRS_Plot_APOE_CSF_pTau217_Abeta_Interaction_BF_2.txt")
...
p1 <- p1 + scale_y_continuous (breaks=seq(0, 6, 0.5))
...
#### Non-APOE region
df_1 <- fread ("/Volumes/ATUL_6TB/Work/Projects/Cell_Specific_PRSs/Full_Data/Manuscript/PRS_Plots/BF_2/PRS_Plot_Non_APOE_CSF_pTau217_Abeta_Interaction.txt")
...
p2 <- p2 + scale_y_continuous (breaks=seq(0, 6, 0.5))
p2 <- p2 + geom_hline (aes(yintercept=-log10(0.05), linetype = "p-value 0.05", col="#F0E442")) +
  geom_hline (aes (yintercept=-log10(1.1E-03), linetype = "Bonferroni p-value 0.05", col="#999999")) +
  scale_linetype_manual(name = "p-value cut off", values = c(2, 2), 
                        guide = guide_legend(override.aes = list(color = c("#999999", "#F0E442"))))
...
annotate_figure(plot, top = text_grob("CSF pTau217 association with Cell−Specific PRS and Aβ interaction: BioFINDER-2", color = "black", face = "bold", size = 16, family = "serif"))
